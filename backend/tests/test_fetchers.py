...
@pytest.mark.asyncio
async def test_fetch_active_fires():
    data = await fetch_active_fires(source="VIIRS_SNPP_NRT", days=1)
...
@pytest.mark.asyncio
async def test_fetch_hrrr_forecast():
    ...
    result = await fetch_hrrr_forecast(lat, lon)
    assert result is None or type(result) is dict

@pytest.mark.asyncio
async def test_fetch_elevetion_data():
    lat, lon = 34.2238, -118.0601
    result = await fetch_elevation_data(lat, lon)
